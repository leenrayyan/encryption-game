...
export function generateTrapSignal(): RoundInstance {
  ...
  // Two genuine messages are prefixed with the codeword; the impostor is not.
  const messages = [
    `${codeword} ${genuinePair[0]}`,
    `${codeword} ${genuinePair[1]}`,
    impostor,
  ];
  const order = shuffle([0, 1, 2]); // randomize which slot is the impostor
  const arranged = order.map((i) => messages[i] as string);
  const impostorSlot = order.indexOf(2); // 0-based index of the impostor after shuffle
...
// The scenario is fixed per session so the whole room faces the same dilemma.
...
/** Helper for the plain decode-and-type rounds (no decision, single answer). */
function simple(
  operator: OperatorPayload,
  crypto: CryptoPayload,
  phrase: string
