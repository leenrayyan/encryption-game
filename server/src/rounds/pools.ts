// Flavor content pools for round generation. Kept separate from logic
// so the game's writing can be tweaked without touching generators.ts.
...
// Each entry is a full transmission plus the branching choice it forces.
export interface FinalScenario {
  message: string; // decoded via pigpen
  prompt: string;
